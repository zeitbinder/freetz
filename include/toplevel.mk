...
HOSTCC ?= gcc
FREETZVERSION:=$(RELEASE)$(if $(REVISION), ($(REVISION)))
export RELEASE
export REVISION
export FREETZVERSION
...
.config: ./scripts/config/conf $(if $(FREETZ_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	if [ \! -e .config ] || ! grep FREETZ_HAVE_DOT_CONFIG .config >/dev/null; then \
		[ -e $(HOME)/.freetz/defconfig ] && cp $(HOME)/.freetz/defconfig .config; \
...
#	if [ \! -e .config -a -e $(HOME)/.freetz/defconfig ]; then \
#		cp $(HOME)/.freetz/defconfig .config; \
#	fi
#	$< Config.in
...
distclean:
	rm -rf tmp build_dir staging_dir dl .config* feeds package/feeds package/freetz-packages bin
