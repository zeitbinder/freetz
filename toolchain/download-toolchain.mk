include toolchain/kernel/ccache/ccache.mk
include toolchain/target/ccache/ccache.mk
include toolchain/target/uclibc/uclibc.mk
...
download-toolchain: $(KERNEL_CROSS_COMPILER) kernel-configured \
			...
			$(CCACHE) $(STDCXXLIB) $(TARGET_CXX_CROSS_COMPILER_SYMLINK_TIMESTAMP) \
			$(if $(FREETZ_PACKAGE_GDB_HOST),gdbhost)

gcc-kernel: $(KERNEL_CROSS_COMPILER)
$(KERNEL_CROSS_COMPILER): $(DL_DIR)/$(KERNEL_TOOLCHAIN_SOURCE)
	$(RM) -r $(TOOLCHAIN_DIR_KERNEL)
	$(STAGING_DIR_HOST)/bin/tar $(TAR_VERBOSITY) -xf $(DL_DIR)/$(KERNEL_TOOLCHAIN_SOURCE) -C $(TOOLCHAIN_DIR)
	@touch $@

gcc: $(TARGET_CROSS_COMPILER)
$(TARGET_CROSS_COMPILER): $(DL_DIR)/$(TARGET_TOOLCHAIN_SOURCE)
	$(RM) -r $(TOOLCHAIN_DIR_TARGET)
	$(STAGING_DIR_HOST)/bin/tar $(TAR_VERBOSITY) -xf $(DL_DIR)/$(TARGET_TOOLCHAIN_SOURCE) -C $(TOOLCHAIN_DIR)
	@touch $@
