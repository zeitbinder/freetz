...
$(PKG)_STAGING_BINARY:=$(STAGING_DIR)/usr/lib/libusb-$($(PKG)_SHORT_VERSION).so.$($(PKG)_LIB_VERSION)
...
$($(PKG)_BINARY): $($(PKG)_DIR)/.configured
	$(PKG_MAKE) -C $(LIBUSB1_DIR) all

$($(PKG)_STAGING_BINARY): $($(PKG)_BINARY)
	$(PKG_MAKE) -C $(LIBUSB1_DIR) \
		DESTDIR="$(STAGING_DIR)" \
		install
	$(PKG_FIX_LIBTOOL_LA) \
		$(STAGING_DIR)/usr/lib/libusb-1.0.la \
		$(STAGING_DIR)/usr/lib/pkgconfig/libusb-1.0.pc \
...
$(pkg)-clean:
	-$(PKG_MAKE) -C $(LIBUSB1_DIR) clean
	$(RM) -r \
		$(STAGING_DIR)/usr/bin/libusb-config \
		$(STAGING_DIR)/usr/include/libusb-1.*\
		$(STAGING_DIR)/usr/lib/libusb-1.* \
		$(STAGING_DIR)/usr/lib/pkgconfig/libusb-1.*.pc
