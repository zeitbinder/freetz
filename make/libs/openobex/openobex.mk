...
$(PKG)_STAGING_BINARY:=$(STAGING_DIR)/usr/lib/libopenobex.so.$($(PKG)_LIB_VERSION)
...
$($(PKG)_STAGING_BINARY): $($(PKG)_BINARY)
	$(PKG_MAKE) -C $(OPENOBEX_DIR) \
		...
		install
	$(PKG_FIX_LIBTOOL_LA) \
		$(STAGING_DIR)/usr/lib/libopenobex.la \
		$(STAGING_DIR)/usr/lib/pkgconfig/openobex.pc
...
$(pkg)-clean:
	-$(PKG_MAKE) -C $(OPENOBEX_DIR) clean
	$(RM) -r $(STAGING_DIR)/usr/lib/libopenobex.* \
		$(STAGING_DIR)/include/openobex \
		$(STAGING_DIR)/usr/lib/pkgconfig/openobex.pc \
		$(STAGING_DIR)/usr/share/aclocal/openobex.m4
